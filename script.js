var padding = { top: 20, right: 0, bottom: 0, left: 0 },
  ...
  vis = container
    .append('g')
    .attr('class', 'spi')
    .attr('transition', 'transform 3s cubic-bezier(0.4, 0, 0.2, 1);')
...
function spin(t) {
  if ((container.on('click', null), oldpick.length != weDATA.length)) {
    for (var e = {}, n = 0; n < weDATA.length; n++) e[n] = weDATA[n].wePercWght;
    var i = weightedRandom(e),
      o = 360 / weDATA.length;
    weDATA.length,
      (rotation = 1440 + o * (weDATA.length - Number(i()))),
      (picked =
        (picked = Math.round(weDATA.length - (rotation % 360) / o)) >=
        weDATA.length
          ? picked % weDATA.length
          : picked),
      -1 === oldpick.indexOf(picked)
        ? (oldpick.push(picked),
          ...
          vis
            ...
            .each('end', function () {
              (document.querySelector('span#prize').innerText =
                weDATA[picked].weName),
                (document.querySelector('code > p').innerText =
                  weDATA[picked].weCode),
                setTimeout(function () {
                  if (
                    (document
                    ...
                    oldpick.length == weDATA.length &&
                      (e.style.display = 'none'),
                      e.addEventListener('click', () => {
                        document
                          .querySelector('.weWinCont.show')
                          .classList.remove('show'),
                          document
                            .querySelector(' dialog > div.spinContainer.hide')
...
function weightedRandom(t) {
  var e,
    n,
    i = [];
  for (e in t) for (n = 0; n < 10 * t[e]; n++) i.push(e);
  return function () {
  ...
  try {
    await navigator.clipboard.writeText(t.innerText),
      ...
      weNotification.trackEvent(
        ...
        JSON.stringify({ 'Coupon Code': t.innerText }),
        !1
      );
  } catch (e) {}
}
arcs
  ...
  svg
    .append('g')
    .attr('transform', 'translate(139,0)')
    .append('path')
    .attr('d', 'M1 1H31V32.615L16 46.6314L1 32.615V1Z')
    .style({
      fill: 'white',
      stroke: '#001D34',
      'stroke-width': '2',
  ...
  svg
    .append('g')
    .attr('transform', 'translate(134.5,-5)')
    .append('circle')
    .attr('cx', 16)
    .attr('cy', 20)
    .attr('r', 5)
    .style({ margin: '10px', position: 'absolute', transform: 'rotate(0deg)' }),
  container
    .append('circle')
    .attr('cx', 0)
    .attr('cy', 0)
